...
-- Опубликованные в Telegram сообщения + счётчики реакций (обучающие метки для trainer)
...
CREATE INDEX IF NOT EXISTS idx_posted_messages_article   ON posted_messages (article_id);
CREATE INDEX IF NOT EXISTS idx_posted_messages_harvest   ON posted_messages (last_reaction_harvested_at);

-- Персистентное состояние (offset для Telegram long-polling)
